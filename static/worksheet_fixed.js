...
function initWorksheetPromptUpdater() {
    ...
    try {
        ...
        // Run additional updates with delays to ensure everything is loaded
        setTimeout(updatePrompt, 500);
        setTimeout(updatePrompt, 1000);
    } catch (error) {
        console.error("Error initializing worksheet prompt updater:", error);
    }
}
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() {
        setTimeout(initWorksheetPromptUpdater, 200);
    });
} else {
    setTimeout(initWorksheetPromptUpdater, 200);
}
